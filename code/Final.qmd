...
ggplot(data = kelp_clean, # select kelp_clean data frame
  ...
  theme( # theme function call
    ...
    panel.grid.major.y = element_line(color = "gray90"), # change gridline colors
    panel.grid.minor.y = element_line(color = "gray90"), # change gridline colors
    panel.grid.major.x = element_line(color = "gray90"), # change gridline colors
    panel.grid.minor.x = element_line(color = "gray90")) # change gridline colors
...
AICc(model0, model1, model2, model3) |> #Akaike's Informatio Criterion code for all models 
...
```{r}
#| label: visualization of model prediction
...
ggplot(mopl_clean, # start with mopl_clean data frame
  ...
  scale_y_continuous(limits = c(0, 1), # make y-axis go from 0 to 1 
                     breaks = c(0, 0.25, 0.5, 0.75, 1)) + # put tick marks only at 0 and 1 
  labs(
...
**Figure 1. Predicted probability of Mountain Plover nest site use as a function of visual obstruction at the nest.**
...
Mountain Plover nest site occupancy was highest when visual obstruction was low. At 0 cm visual obstruction, the predicted probability of nest site occupancy was 0.73 (95% CI: 0.65–0.80), indicating that plovers strongly prefer open nesting areas. As shown in Figure 1, the probability of nest site use decreases as visual obstruction increases, suggesting that taller or denser vegetation reduces habitat suitability. This pattern likely occurs because Mountain Plovers nest in open habitats with short, sparse vegetation and large amounts of bare ground, which allows them to detect predators and blend into the surrounding soil. Prairie dogs help create these open habitat conditions by clipping vegetation and disturbing soil, making prairie dog colonies especially suitable nesting habitat for Mountain Plovers.
...
-   What patterns (e.g. differences in means/counts/proportions/medians, trends through time, relationships between variables) do you see in each visualization? Are these different between visualizations? If so, why? If not, why not?
    ...
    Higher median screen time for school days and lower screen time for non-school days are depicted and are same in both of my visualizations. They are both illustrated based on the same data/results, and individual observations between school and non-school days are also delineated similiarily as well. The medium of communication is different, but both visualizations show the same pattern/trend from personal data collection
